...
func ClipParameters(w []float64, numRelearningSteps int, enableShortTerm bool) []float64 {
	...
	// W[7]: mean reversion
	result[7] = Clamp(result[7], 0.0, 1.0)

	// W[8]: stability growth base
	result[8] = Clamp(result[8], 0.0, 6.0)

	// W[9]: stability growth exponent
	result[9] = Clamp(result[9], 0.0, 2.0)

	// W[10]: retrievability factor
	result[10] = Clamp(result[10], 0.0, 4.0)

	// W[11]: forget stability base
	result[11] = Clamp(result[11], 0.001, 3.5)

	// W[12]: forget difficulty factor
	result[12] = Clamp(result[12], 0.001, 0.5)

	// W[13]: forget stability exponent
	result[13] = Clamp(result[13], 0.001, 0.8)
	...
	// W[20]: decay
	result[20] = Clamp(result[20], 0.01, 0.8)
